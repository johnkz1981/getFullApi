...
define( 'ERROR_MSGS', '0' ); 		// Вывод сообщений: 					1- включен, 0 - отключен
define( 'SPEED_TEST', '0' ); 		// Замер скорости выполнения скрипта: 	1- включен, 0 - отключен

define( 'HOST', 'localhost' ); 		// Сервер баз данных
define( 'PORT', '3306' ); 			// Порт сервера баз данных
define( 'USER', 'root' ); 			// Пользователь базы данных
define( 'PASS', '>ufdnjltnfkm!*' ); 		// Пароль пользователя
define( 'NAME', 'tecdoc' ); 		// Имя базы данных
...
class Tecdoc extends DataBase {
	...
	static function getSupplierId($supplier){
		$idSupplires= parent::select("SELECT id FROM suppliers where description = '$supplier'");
		if(isset($idSupplires[0]->id)) $idSupplires=$idSupplires[0]->id;
			else $idSupplires = null;
		return $idSupplires;
	}

	static function getArticle($articul){
		$ArticleNumber= parent::select("SELECT DataSupplierArticleNumber FROM articles where FoundString = '$articul'");
		if(isset($ArticleNumber[0]->DataSupplierArticleNumber)) $ArticleNumber=$ArticleNumber[0]->DataSupplierArticleNumber;
		else $ArticleNumber = null;
		return $ArticleNumber;
	}

	static function isArticles($supplier,$articul){
		$supplierId=self::getSupplierId($supplier);
		$articles= parent::select("select  supplierId, DataSupplierArticleNumber from articles where supplierId ='".$supplierId."' and FoundString = '".$articul."'");
		return $articles[0]==null?false:true;
	}
...
//===================================//	
	...
	static function getArtAttributes( $number, $brand_id )
		{
			return parent::select("
					SELECT description, displaytitle, displayvalue FROM article_attributes WHERE datasupplierarticlenumber='" . $number . "'  AND supplierid='" . $brand_id . "'
				");
		}	
